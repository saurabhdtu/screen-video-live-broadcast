...
public class NewSwitchVideoInputActivity extends BaseActivity {
    private static final String TAG = NewSwitchVideoInputActivity.class.getSimpleName();
    ...
    private NewScreenShareService.ScreenShareBinder mService;
    private VideoInputServiceConnection mServiceConnection;


    private void bindVideoService(Intent intent) {
        intent.setClass(this, NewScreenShareService.class);
        DisplayMetrics metrics = new DisplayMetrics();
        getWindowManager().getDefaultDisplay().getMetrics(metrics);
        intent.putExtra(ScreenInputManager.FLAG_SCREEN_WIDTH, metrics.widthPixels);
        intent.putExtra(ScreenInputManager.FLAG_SCREEN_HEIGHT, metrics.heightPixels);
        intent.putExtra(ScreenInputManager.FLAG_SCREEN_DPI, (int) metrics.density);
        intent.putExtra(ScreenInputManager.FLAG_FRAME_RATE, 24);
...
//            rtcEngine().setExternalVideoSource(true, true, true);
    ...
    private class VideoInputServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            mService = (NewScreenShareService.ScreenShareBinder) iBinder;
            start = true;
        }
